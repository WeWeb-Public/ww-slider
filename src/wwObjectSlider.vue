    ...
    style() {
      ...
      if (this.wwAttrs && this.wwAttrs.wwCategory == "background") {
        ...
        if (this.wwObject.content.data.fitContent && this.$el) {
          const slidesContent = this.$el.querySelectorAll(".inner-content");
          let maxHeight = 0;
          for (const slideContent of slidesContent) {
            if (maxHeight < slideContent.getBoundingClientRect().height) {
              maxHeight = slideContent.getBoundingClientRect().height;
            }
          }
          ...
          fields: [
            {
              label: {
                ...
                fr: "S'adapter à la taille de la plus grande slide :",
              },
              desc: {
